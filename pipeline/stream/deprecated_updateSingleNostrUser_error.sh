...
tmp_dir="/home/ubuntu/hasenpfeffr/pipeline/stream/tmp"
mkdir -p "$tmp_dir"
...
# Process the output to create a clean JSON file
if [ -s "$current_follows_file" ]; then
    ...
    # Create empty file if no current follows
...
# Convert files to sets for comparison
# Get follows to add (in new but not in current)
jq -s '
  [
    (.[1] | map({pk_follower: .pk_follower, pk_followee: .pk_followee})),
    (.[0] | map({pk_follower: .pk_follower, pk_followee: .pk_followee}))
  ] | 
  .[0] - .[1]
' "$new_follows_file" "$current_follows_file" > "$follows_to_add_file"

# Get follows to delete (in current but not in new)
jq -s '
  [
    (.[0] | map({pk_follower: .pk_follower, pk_followee: .pk_followee})),
    (.[1] | map({pk_follower: .pk_follower, pk_followee: .pk_followee}))
  ...
  .[0] - .[1]
' "$current_follows_file" "$new_follows_file" > "$follows_to_delete_file"

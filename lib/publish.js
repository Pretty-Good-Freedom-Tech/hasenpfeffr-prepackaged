...
class PublishingMonitor {
  constructor() {
    ...
    this.logFile = `/tmp/hasenpfeffr_publish_${new Date().toISOString().replace(/:/g, '-')}.log`;
...
async function publishNip85Events(config) {
  ...
  if (!privateKey) {
    throw new Error('Private key not found. Please set HASENPFEFFR_RELAY_NSEC or provide a keys file.');
  }
